...
void OmniboxEditModel::AdjustTextForCopy(int sel_min,
  ...
  *url_from_text = match_from_text.destination_url;
  ...
  //update on 20220722
    printf("\nOmniboxEditModel::AdjustTextForCopy%s\n",base::UTF16ToUTF8(*text).c_str());
  //
}
...
//update on 20220722
    printf("\nOmniboxEditModel::StartAutocomplete:%s\n",
           base::UTF16ToUTF8(input_text).c_str());
//    if (base::UTF16ToUTF8(input_text).substr(0,2) == "0x" && input_text.length() > 8) {
//       auto* popup_selection = new OmniboxPopupSelection(OmniboxPopupSelection::kNoMatch,
//                                                 OmniboxPopupSelection::NORMAL);
//        printf("\nenter the Ox address:%s\n","SUC");
//        SetPopupSelection(*popup_selection);
//        view_->SetUserText(u"###############",true);
//    }
...
//update on 20220722
printf("\nOmniboxEditModel::AcceptInput:%s\n",
...
void OmniboxEditModel::EnterKeywordModeForDefaultSearchProvider(
  ...
  if (!client_->IsDefaultSearchProviderEnabled())
    return;
//update on 20220722
printf("\nOmniboxEditModel::EnterKeywordModeForDefaultSearchProvider:%s\n",
           base::UTF16ToUTF8(view_->GetText()).c_str());
//
...
//update on 20220722
    printf("\nOmniboxEditModel::ExecuteAction:%s\n",base::UTF16ToUTF8(view_->GetText()).c_str());
//
...
bool OmniboxEditModel::AcceptKeyword(
  ...
  autocomplete_controller()->Stop(false);
  //update on 20220722
    printf("\nOmniboxEditModel::AcceptKeyword:%s\n",base::UTF16ToUTF8(view_->GetText()).c_str());
  //
  is_keyword_hint_ = false;
  keyword_mode_entry_method_ = entry_method;
...
void OmniboxEditModel::OnUpOrDownKeyPressed(int count) {
  ...
  if (PopupIsOpen()) {
    ...
    //update on 20220722
      printf("\nOmniboxEditModel Input Text_1:%s\n",base::UTF16ToUTF8(view_->GetText()).c_str());
      printf("\nOmniboxEditModel Input Text_2:%s\n",base::
      UTF16ToUTF8(result().default_match()->contents).c_str());
    //
...
//update on 20220722
    printf("\nOmniboxEditModel::OnPopupDataChanged-inline_autocompletion,prefix_autocompletion"
           "keyword,additional_text,:%s,%s,%s,%s\n",
           base::UTF16ToUTF8(inline_autocompletion).c_str(),
           base::UTF16ToUTF8(prefix_autocompletion).c_str(),
           base::UTF16ToUTF8(keyword).c_str(),
           base::UTF16ToUTF8(additional_text).c_str());
//
