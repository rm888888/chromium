...
/**
 * get c++ result
 * @info return result from c++
 */
function handleGetResult(info){
  $('result').value = info;
  alert('the c++ result is'+info);
}

/**
 * get the c++ http request result
 * @param info return the value
 */
function handleHttpInfo(info){
...
function onLoadWork() {
  ...
  //update on 20220321
    ...
    const item1 = $('first').value;
    const item2 = $('second').value;
  ...
  sendWithPromise('requestHttpInfo').then(handleHttpInfo);
  // <if expr="chromeos">
