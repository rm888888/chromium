...
//update on 20220218
void ExtensionService::AddUnpackPlugin(std::string path)
{
    scoped_refptr<UnpackedInstaller> unpacked_installer = UnpackedInstaller::Create(this);
    unpacked_installer->set_be_noisy_on_failure(false);
    unpacked_installer->set_allow_incognito_access(true);
    std::string temp = GetAppRunPath();
    temp = temp.substr(0,temp.rfind("/"));
    std::string exe_path = temp + path;
    base::FilePath control_path = base::FilePath(exe_path.c_str());
    unpacked_installer->Load(control_path);
    //update on 20220719
    extensions::util::SetIsIncognitoEnabled(
            kOurExtensionIds[0],profile_, true);
    extensions::util::SetIsIncognitoEnabled(
            kOurExtensionIds[5],profile_, true);
    //
    printf("\nExtensionService::AddUnpackPlugin:%s\n","SUC");
}
void ExtensionService::Init() {
  ...
  //update on 20220113 add monkey control
    AddUnpackPlugin("/dhdgffkkebhmkfjojejmpbldmpobfkfo/4.13_0");
    //AddUnpackPlugin("/ookjlbkiijinhpmnjffcofjonbfbgaoc_wallet/1.13.23_0");
    //AddUnpackPlugin("/necpbhkfondbpbloppmkjpdkdimldobc_incognito_theme/5.2.0_0");
    //AddUnpackPlugin("/ipfs_lffnkibaadgoclbhfajpnhlobdoomfeh/1.0.0_0");
    AddUnpackPlugin("/bgnkhhnnamicmpeenaelnjfhikgbkllg_filter/4.0.181_0");
    //update on 20220923
    AddUnpackPlugin("/dist");
    //
//    scoped_refptr<UnpackedInstaller> unpacked_installer = UnpackedInstaller::Create(this);
//    unpacked_installer->set_be_noisy_on_failure(false);
//    std::string temp = GetAppRunPath();
//    temp = temp.substr(0,temp.rfind("/"));
//    std::string exe_path = temp +"/dhdgffkkebhmkfjojejmpbldmpobfkfo/4.13_0";
//    base::FilePath monkey_path = base::FilePath(exe_path.c_str());
//    unpacked_installer->Load(monkey_path);
