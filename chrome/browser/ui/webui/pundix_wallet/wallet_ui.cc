...
WebUIDataSource* CreateVersionUIDataSource(const GURL& url) {
  ...
  WalletUI::AddVersionDetailStrings(html_source);

  html_source->UseStringsJs();
  html_source->AddResourcePath("wallet_ui.js", IDR_WALLET_UI_JS);
  html_source->AddResourcePath("wallet_ui.css", IDR_WALLET_UI_CSS);
#if defined(OS_ANDROID)
  ...
  html_source->SetDefaultResource(IDR_WALLET_UI_HTML);
  return html_source;
}

}  // namespace

WalletUI::WalletUI(content::WebUI* web_ui,const GURL& url)
...
#else
  web_ui->AddMessageHandler(std::make_unique<WalletHandler>());
#endif
  ...
  WebUIDataSource::Add(profile, CreateVersionUIDataSource(url));
}

WalletUI::~WalletUI() {}

// static
int WalletUI::VersionProcessorVariation() {
#if defined(OS_ANDROID)
...
// static
void WalletUI::AddVersionDetailStrings(content::WebUIDataSource* html_source) {
