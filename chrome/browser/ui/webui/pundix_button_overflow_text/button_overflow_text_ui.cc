...
WebUIDataSource* CreateUIDataSource(const GURL& url) {
  ...
  //
  html_source->AddResourcePath("button_overflow_text_ui.js", IDR_BUTTON_OVERFLOW_TEXT_UI_JS);
  html_source->AddResourcePath("button_overflow_text_ui.css", IDR_BUTTON_OVERFLOW_TEXT_UI_CSS);

  html_source->SetDefaultResource(IDR_BUTTON_OVERFLOW_TEXT_UI_HTML);
...
ButtonOverflowTextUI::ButtonOverflowTextUI(content::WebUI* web_ui,const GURL& url)
...
#else
  web_ui->AddMessageHandler(std::make_unique<ButtonOverflowTextHandler>());
#endif
  WebUIDataSource::Add(profile, CreateUIDataSource(url));
    printf("\nButtonOverflowTextUI::ButtonOverflowTextUI suc\n");
}

ButtonOverflowTextUI::~ButtonOverflowTextUI() {}
