...
//update on 20220315
#include "chrome/browser/ui/webui/pundix_wallet/wallet_ui.h"
namespace {

class LeftSidePanelWebView : public views::WebView,
                                  public TabStripModelObserver,
                                  public BubbleContentsWrapper::Host {
 public:
   LeftSidePanelWebView(Browser* browser, base::RepeatingClosure close_cb)
      : browser_(browser),
        close_cb_(std::move(close_cb)),
        contents_wrapper_(std::make_unique<BubbleContentsWrapperT<ReadLaterUI>>(
            GURL(chrome::kPundixWalletURL),
            browser->profile(),
 ...
 private:
  ...
  base::WeakPtrFactory<LeftSidePanelWebView> weak_factory_{this};
};

}  // namespace

LeftSideToolbarButton::LeftSideToolbarButton(Browser* browser)
    : ToolbarButton(base::BindRepeating(&LeftSideToolbarButton::ButtonPressed,
...
LeftSideToolbarButton::~LeftSideToolbarButton(){
  //update on 20220315
  //HideSidePanel();
}

LeftSideToolbarButton::DotBoundsUpdater::DotBoundsUpdater(
...
LeftSideToolbarButton::DotBoundsUpdater::~DotBoundsUpdater() = default;

void LeftSideToolbarButton::DotBoundsUpdater::OnViewBoundsChanged(
    View* observed_view) {
...
void LeftSideToolbarButton::ReadingListModelLoaded(
    const ReadingListModel* model) {
  if (model->unseen_size())
    dot_indicator_->Show();
}

void LeftSideToolbarButton::ReadingListModelBeingDeleted(
    const ReadingListModel* model) {
...
void LeftSideToolbarButton::ReadingListDidApplyChanges(
...
void LeftSideToolbarButton::ButtonPressed() {
  BrowserView* const browser_view =
      BrowserView::GetBrowserViewForBrowser(browser_);
  DCHECK(browser_view->left_aligned_side_panel_for_testing());
  ...
  if (!side_panel_webview_) {
    ...
    // destroy the LeftSideToolbarButton.
    auto webview = std::make_unique< LeftSidePanelWebView>(
        browser_, base::BindRepeating(&LeftSideToolbarButton::ButtonPressed,
                                      base::Unretained(this)));
    side_panel_webview_ =
        browser_view->left_aligned_side_panel_for_testing()->AddChildView(
            std::move(webview));
...
void LeftSideToolbarButton::HideSidePanel() {
  BrowserView* const browser_view =
      BrowserView::GetBrowserViewForBrowser(browser_);
  DCHECK(browser_view->left_aligned_side_panel_for_testing());
  if (side_panel_webview_) {
    browser_view->left_aligned_side_panel_for_testing()->RemoveChildViewT(
        side_panel_webview_);
...
bool LeftSideToolbarButton::ShouldShowInkdropAfterIphInteraction() {
  return false;
}
