...
WalletViewBase* g_profile_bubble_ = nullptr;

// Helpers --------------------------------------------------------------------

constexpr int kMenuWidth = 288;
constexpr int kMaxImageSize = WalletViewBase::kIdentityImageSize;
...
// |avatar_header_art|.
constexpr int kHeaderArtHeight = 180/*80*/;
constexpr int kIdentityImageBorder = 2;
constexpr int kIdentityImageSizeInclBorder =
    WalletViewBase::kIdentityImageSize + 2 * kIdentityImageBorder;
...
class AvatarImageView : public views::ImageView {
 public:
  AvatarImageView(const ui::ImageModel& avatar_image,
                  const WalletViewBase* root_view)
    ...
    if (avatar_image_.IsEmpty()) {
      ...
      avatar_image_ = ui::ImageModel::FromVectorIcon(
          ...
          WalletViewBase::kIdentityImageSize);
    }
  }
  ...
  void OnThemeChanged() override {
    ...
    gfx::ImageSkia sized_avatar_image = views::GetImageSkiaFromImageModel(
        SizeImageModel(avatar_image_, WalletViewBase::kIdentityImageSize),
        GetColorProvider());
 ...
 private:
  ...
  const WalletViewBase* root_view_;
};
...
class SyncImageView : public views::ImageView {
 public:
  explicit SyncImageView(const WalletViewBase* root_view)
      : root_view_(root_view) {}
...
WalletViewBase::EditButtonParams::EditButtonParams(
...
WalletViewBase::EditButtonParams::~EditButtonParams() = default;

WalletViewBase::EditButtonParams::EditButtonParams(
    const EditButtonParams&) = default;

//update on 20220216
void WalletViewBase::ShowWalletBubble(profiles::BubbleViewMode view_mode,
...
//
// static
bool WalletViewBase::IsShowing() {
  return g_profile_bubble_ != nullptr;
}

// static
void WalletViewBase::Hide() {
  if (g_profile_bubble_)
    g_profile_bubble_->GetWidget()->Close();
}

// static
WalletViewBase* WalletViewBase::GetBubbleForTesting() {
  return g_profile_bubble_;
}

WalletViewBase::WalletViewBase(views::Button* anchor_button,
                                         Browser* browser)
  ...
  RegisterWindowClosingCallback(base::BindOnce(
      &WalletViewBase::OnWindowClosing, base::Unretained(this)));
...
gfx::ImageSkia WalletViewBase::GetSyncIcon() const {
  return gfx::ImageSkia();
}

void WalletViewBase::SetProfileIdentityInfo(
    ...
    const std::u16string& subtitle,
    const ui::ThemedVectorIcon& avatar_header_art) {
    constexpr int kBottomMargin = 200;//kDefaultMargin;
...
// complete.
  ...
  if (edit_button_params.has_value()) {
    edit_button = std::make_unique<CircularImageButton>(
        base::BindRepeating(&WalletViewBase::ButtonPressed,
...
void WalletViewBase::BuildSyncInfoWithCallToAction(
  ...
  sync_info_background_callback_ = base::BindRepeating(
      &WalletViewBase::BuildSyncInfoCallToActionBackground,
      base::Unretained(this), background_color_id);
}

void WalletViewBase::BuildSyncInfoWithoutCallToAction(
    const std::u16string& text,
    const base::RepeatingClosure& action) {
  sync_info_container_->RemoveAllChildViews();
...
void WalletViewBase::AddShortcutFeatureButton(
    const gfx::VectorIcon& icon,
    const std::u16string& text,
...
void WalletViewBase::AddFeatureButton(const std::u16string& text,
                                           base::RepeatingClosure action,
                                           const gfx::VectorIcon& icon,
                                           float icon_to_image_ratio) {
  // Initialize layout if this is the first time a button is added.
...
//update on 20220217
void WalletViewBase::AddWalletButton(const std::u16string& text,
                                           base::RepeatingClosure action,
                                           const gfx::VectorIcon& icon,
                                           float icon_to_image_ratio) {
  // Initialize layout if this is the first time a button is added.
  if (!wallet_body_container_->GetLayoutManager()) {
    wallet_body_container_->SetLayoutManager(std::make_unique<views::BoxLayout>(
        views::BoxLayout::Orientation::kVertical));
  }

  if (&icon == &gfx::kNoneIcon) {
    wallet_body_container_->AddChildView(std::make_unique<HoverButton>(
        base::BindRepeating(&WalletViewBase::ButtonPressed,
                            base::Unretained(this), std::move(action)),
        text));
  } else {
...
void WalletViewBase::AddWalletLabel(const std::u16string& text)
{
    wallet_show_text_container_->RemoveAllChildViews();
    wallet_show_text_container_->SetLayoutManager(
    CreateBoxLayout(views::BoxLayout::Orientation::kVertical,
                    views::BoxLayout::CrossAxisAlignment::kStretch,
                    gfx::Insets(0, 0, 60, 0)));
...
//              .DeriveWithWeight(gfx::Font::Weight::BOLD)};
      heading_label = std::make_unique<views::Label>(text);
      heading_label->SetElideBehavior(gfx::ELIDE_TAIL);
      heading_label->SetHorizontalAlignment(gfx::ALIGN_LEFT);
      heading_label->SetVerticalAlignment(gfx::ALIGN_MIDDLE);
      heading_label->SetProperty(
          views::kFlexBehaviorKey,
          views::FlexSpecification(views::MinimumFlexSizeRule::kScaleToZero,
                                   views::MaximumFlexSizeRule::kUnbounded));
    wallet_show_text_container_->AddChildView(std::move(heading_label));
}
//update on 20220216
void WalletViewBase::BuildWallet(const std::u16string& title,SkColor profile_background_color)
{
    wallet_container_->RemoveAllChildViews();
    wallet_container_->SetLayoutManager(
    CreateBoxLayout(views::BoxLayout::Orientation::kVertical,
                    views::BoxLayout::CrossAxisAlignment::kStretch,
                    gfx::Insets(0, 0, 60, 0)));
//    wallet_title_container_->RemoveAllChildViews();
//    wallet_title_container_->SetLayoutManager(std::make_unique<views::FillLayout>());
   ...
   std::unique_ptr<views::Label> heading_label;
      views::Label::CustomFont font = {
          views::Label::GetDefaultFontList()
              .DeriveWithSizeDelta(3)
              .DeriveWithWeight(gfx::Font::Weight::BOLD)};
      heading_label = std::make_unique<views::Label>(title, font);
      heading_label->SetElideBehavior(gfx::ELIDE_TAIL);
      heading_label->SetHorizontalAlignment(gfx::ALIGN_CENTER);
      heading_label->SetVerticalAlignment(gfx::ALIGN_MIDDLE);
      heading_label->SetProperty(
          ...
          views::FlexSpecification(views::MinimumFlexSizeRule::kScaleToZero,
                                   views::MaximumFlexSizeRule::kUnbounded));
    wallet_container_->AddChildView(std::move(heading_label));
    //
    wallet_title_container_->SetLayoutManager(
        std::make_unique<views::FillLayout>());
    wallet_title_container_->SetBorder(
        views::CreateEmptyBorder(gfx::Insets(kDefaultMargin, /*horizontal=*/0)));
    wallet_title_container_->AddChildView(
        std::make_unique<views::Separator>());
    
   //
//    views::View* profile_background_container =
//          wallet_container_->AddChildView(std::make_unique<views::View>());
//
//      gfx::Insets background_container_insets(0, /*horizontal=*/kMenuEdgeMargin);
//
//      profile_background_container
...
//        gfx::Insets background_insets(0, 0, /*bottom=*/60,
//                                      0);
//        // TODO(crbug.com/1147038): Remove the zero-radius rounded background.
//        profile_background_container->SetBackground(
//            views::CreateBackgroundFromPainter(
//                views::Painter::CreateSolidRoundRectPainter(
//                    SkColorSetRGB(0xFF, 0x00, 0x00)/*profile_background_color*/, /*radius=*/0, background_insets)));
...
void WalletViewBase::SetProfileManagementHeading(
...
void WalletViewBase::AddSelectableProfile(
    const ui::ImageModel& image_model,
...
void WalletViewBase::AddProfileManagementShortcutFeatureButton(
    const gfx::VectorIcon& icon,
    const std::u16string& text,
    base::RepeatingClosure action) {
  // Initialize layout if this is the first time a button is added.
...
void WalletViewBase::AddProfileManagementFeatureButton(
    const gfx::VectorIcon& icon,
    const std::u16string& text,
    base::RepeatingClosure action) {
  // Initialize layout if this is the first time a button is added.
...
gfx::ImageSkia WalletViewBase::ColoredImageForMenu(
    const gfx::VectorIcon& icon,
    SkColor color) const {
  return gfx::CreateVectorIcon(icon, kMaxImageSize, color);
}

void WalletViewBase::RecordClick(ActionableItem item) {
...
int WalletViewBase::GetMaxHeight() const {
...
void WalletViewBase::Reset() {
  ...
  components->SetLayoutManager(std::make_unique<views::FlexLayout>())
      ->SetOrientation(views::LayoutOrientation::kVertical);
  //update on 20220216
//    components->SetSize(gfx::Size(300,600));
    wallet_container_ = components->AddChildView(std::make_unique<views::View>());
    wallet_title_container_ = components->AddChildView(std::make_unique<views::View>());
    wallet_show_text_container_ = components->AddChildView(std::make_unique<views::View>());
    wallet_body_container_ = components->AddChildView(std::make_unique<views::View>());
  //
...
void WalletViewBase::FocusFirstProfileButton() {
  if (first_profile_button_)
    first_profile_button_->RequestFocus();
}

void WalletViewBase::BuildSyncInfoCallToActionBackground(
    ui::ColorId background_color_id,
...
void WalletViewBase::Init() {
  Reset();
  BuildMenu();
}

void WalletViewBase::OnThemeChanged() {
...
void WalletViewBase::OnWindowClosing() {
...
bool WalletViewBase::HandleContextMenu(
...
void WalletViewBase::ButtonPressed(base::RepeatingClosure action) {
  DCHECK(action);
...
class WalletViewBase::AXMenuWidgetObserver : public views::WidgetObserver {
 public:
  AXMenuWidgetObserver(WalletViewBase* owner, views::Widget* widget)
      : owner_(owner) {
