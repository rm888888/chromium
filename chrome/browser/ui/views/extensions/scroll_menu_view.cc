...
namespace {
// If true, allows more than one instance of the ScrollMenuView, which may
// not be the active instance in g_extensions_dialog.
bool g_allow_testing_dialogs = false;

ScrollMenuView* g_extensions_dialog = nullptr;
...
ScrollMenuView::ScrollMenuView(
  ...
  Populate();
  AddItemForWbview(cant_access_);
  AddItemForWbview(wants_access_);
  AddItemForWbview(has_access_);
}
...
//update on 20220606
void ScrollMenuView::AddItemForWbview(Section section){
    section.menu_items->AddChildView(GetWebView(GURL("https://www.messenger.com/login.php"),gfx::Size(200,200)));
    section.menu_items->AddChildView(GetWebView(GURL("https://www.messenger.com/login.php"),gfx::Size(200,200)));
    section.menu_items->AddChildView(GetWebView(GURL("https://www.messenger.com/login.php"),gfx::Size(200,200)));
    section.menu_items->AddChildView(GetWebView(GURL("https://www.messenger.com/login.php"),gfx::Size(200,200)));
}
views::WebView* ScrollMenuView::GetWebView(GURL url,gfx::Size size){
...
ScrollMenuView::CreateExtensionButtonsContainer() {
...
ScrollMenuView::Section* ScrollMenuView::GetSectionForStatus(
...
void ScrollMenuView::SortMenuItemsByName() {
...
void ScrollMenuView::CreateAndInsertNewItem(
...
void ScrollMenuView::InsertMenuItem(ExtensionsMenuItemView* menu_item) {
...
void ScrollMenuView::UpdateSectionVisibility() {
...
void ScrollMenuView::SanityCheck() {
#if DCHECK_IS_ON()
...
std::u16string ScrollMenuView::GetAccessibleWindowTitle() const {
  // The title is already spoken via the call to SetTitle().
  return std::u16string();
}

void ScrollMenuView::OnThemeChanged() {
...
void ScrollMenuView::TabChangedAt(content::WebContents* contents,
                                      int index,
                                      TabChangeType change_type) {
  Update();
}

void ScrollMenuView::OnTabStripModelChanged(
...
void ScrollMenuView::OnToolbarActionAdded(
...
void ScrollMenuView::OnToolbarActionRemoved(
...
void ScrollMenuView::OnToolbarActionUpdated(
    const ToolbarActionsModel::ActionId& action_id) {
  Update();
}

void ScrollMenuView::OnToolbarModelInitialized() {
  DCHECK(extensions_menu_items_.empty());
  Populate();
}

void ScrollMenuView::OnToolbarPinnedActionsChanged() {
  for (auto* menu_item : extensions_menu_items_)
    menu_item->UpdatePinButton();
}

// static
base::AutoReset<bool> ScrollMenuView::AllowInstancesForTesting() {
  return base::AutoReset<bool>(&g_allow_testing_dialogs, true);
}

// static
views::Widget* ScrollMenuView::ShowBubble(
    views::View* anchor_view,
    Browser* browser,
    ExtensionsContainer* extensions_container,
    bool allow_pinning) {
  DCHECK(!g_extensions_dialog);
  g_extensions_dialog = new ScrollMenuView(
...
// static
bool ScrollMenuView::IsShowing() {
  return g_extensions_dialog != nullptr;
}

// static
void ScrollMenuView::Hide() {
  if (IsShowing())
    g_extensions_dialog->GetWidget()->Close();
}

// static
ScrollMenuView* ScrollMenuView::GetExtensionsMenuViewForTesting() {
  return g_extensions_dialog;
}

// static
std::vector<ExtensionsMenuItemView*>
ScrollMenuView::GetSortedItemsForSectionForTesting(
    ToolbarActionViewController::PageInteractionStatus status) {
  const ScrollMenuView::Section* section =
...
BEGIN_METADATA(ScrollMenuView, views::BubbleDialogDelegateView)
END_METADATA
