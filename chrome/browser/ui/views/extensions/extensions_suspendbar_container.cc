...
void ExtensionsSuspendbarContainer::SetOnVisibleCallbackForTesting(
    base::OnceClosure callback) {
  GetOnVisibleCallbackForTesting() = std::move(callback);
}

struct ExtensionsSuspendbarContainer::DropInfo {
...
ExtensionsSuspendbarContainer::DropInfo::DropInfo(
    ToolbarActionsModel::ActionId action_id,
    size_t index)
    : action_id(action_id), index(index) {}

ExtensionsSuspendbarContainer::ExtensionsSuspendbarContainer(Browser* browser,
...
ExtensionsSuspendbarContainer::~ExtensionsSuspendbarContainer() {
    //update on 20220221
...
void ExtensionsSuspendbarContainer::UpdateAllIcons() {
...
ExtensionsToolbarButton* ExtensionsSuspendbarContainer::GetExtensionsButton()
    const {
  return extensions_button_ ? extensions_button_
                            : extensions_controls_->extensions_button();
}

ToolbarActionView* ExtensionsSuspendbarContainer::GetViewForId(
    const std::string& id) {
  const auto it = icons_.find(id);
  return (it == icons_.end()) ? nullptr : it->second;
}

void ExtensionsSuspendbarContainer::ShowWidgetForExtension(
    views::Widget* widget,
  ...
  GetAnimatingLayoutManager()->PostOrQueueAction(base::BindOnce(
      &ExtensionsSuspendbarContainer::AnchorAndShowWidgetImmediately,
      weak_ptr_factory_.GetWeakPtr(), widget));
}

views::Widget*
ExtensionsSuspendbarContainer::GetAnchoredWidgetForExtensionForTesting(
    const std::string& extension_id) {
...
bool ExtensionsSuspendbarContainer::ShouldForceVisibility(
...
void ExtensionsSuspendbarContainer::UpdateIconVisibility(
    const std::string& extension_id) {
...
void ExtensionsSuspendbarContainer::AnchorAndShowWidgetImmediately(
    views::Widget* widget) {
...
ToolbarActionViewController* ExtensionsSuspendbarContainer::GetActionForId(
...
ToolbarActionViewController* ExtensionsSuspendbarContainer::GetPoppedOutAction()
    const {
  return popped_out_action_;
}

void ExtensionsSuspendbarContainer::OnContextMenuShown(
...
void ExtensionsSuspendbarContainer::OnContextMenuClosed(
    ToolbarActionViewController* extension) {
...
bool ExtensionsSuspendbarContainer::IsActionVisibleOnToolbar(
...
ExtensionsSuspendbarContainer::GetActionVisibility(
    const ToolbarActionViewController* action) const {
...
void ExtensionsSuspendbarContainer::UndoPopOut() {
...
void ExtensionsSuspendbarContainer::SetPopupOwner(
...
void ExtensionsSuspendbarContainer::HideActivePopup() {
  if (popup_owner_)
...
bool ExtensionsSuspendbarContainer::CloseOverflowMenuIfOpen() {
  if (ExtensionsMenuView::IsShowing()) {
...
void ExtensionsSuspendbarContainer::PopOutAction(
...
bool ExtensionsSuspendbarContainer::ShowToolbarActionPopupForAPICall(
    const std::string& action_id) {
...
void ExtensionsSuspendbarContainer::ShowToolbarActionBubble(
...
void ExtensionsSuspendbarContainer::ShowToolbarActionBubbleAsync(
    std::unique_ptr<ToolbarActionsBarBubbleDelegate> bubble) {
  ShowToolbarActionBubble(std::move(bubble));
}

void ExtensionsSuspendbarContainer::ToggleExtensionsMenu() {
  GetExtensionsButton()->ToggleExtensionsMenu();
    printf("ExtensionsSuspendbarContainer::ToggleExtensionsMenu():%s","start");
}

bool ExtensionsSuspendbarContainer::HasAnyExtensions() const {
  return !actions_.empty();
}

void ExtensionsSuspendbarContainer::OnTabStripModelChanged(
...
void ExtensionsSuspendbarContainer::OnToolbarActionAdded(
...
void ExtensionsSuspendbarContainer::OnToolbarActionRemoved(
    const ToolbarActionsModel::ActionId& action_id) {
...
void ExtensionsSuspendbarContainer::OnToolbarActionUpdated(
    const ToolbarActionsModel::ActionId& action_id) {
  ToolbarActionViewController* action = GetActionForId(action_id);
  if (action)
    action->UpdateState();
...
void ExtensionsSuspendbarContainer::OnToolbarModelInitialized() {
  CreateActions();
  //update on 20220510
  ToggleExtensionsMenu();
}

void ExtensionsSuspendbarContainer::OnToolbarPinnedActionsChanged() {
...
void ExtensionsSuspendbarContainer::ReorderViews() {
...
void ExtensionsSuspendbarContainer::CreateActions() {
    //update on 20220510
//  DCHECK(icons_.empty());
...
void ExtensionsSuspendbarContainer::CreateActionForId(
    const ToolbarActionsModel::ActionId& action_id) {
    //update on 20220510
...
//      ExtensionActionViewController::Create(action_id, browser_, this));
  ...
  //update on 20220525
      ...
      if(!main_item && (main_item->GetID() == 888888))
          main_item->SetVisible(false);
}

content::WebContents* ExtensionsSuspendbarContainer::GetCurrentWebContents() {
  return browser_->tab_strip_model()->GetActiveWebContents();
}

bool ExtensionsSuspendbarContainer::CanShowIconInToolbar() const {
  // Pinning extensions is not available in PWAs.
  return !browser_->app_controller();
}

views::LabelButton* ExtensionsSuspendbarContainer::GetOverflowReferenceView()
    const {
  return GetExtensionsButton();
}

gfx::Size ExtensionsSuspendbarContainer::GetToolbarActionSize() {
  constexpr gfx::Size kDefaultSize(80, 80);
...
void ExtensionsSuspendbarContainer::WriteDragDataForView(
    View* sender,
...
int ExtensionsSuspendbarContainer::GetDragOperationsForView(View* sender,
                                                         const gfx::Point& p) {
  return browser_->profile()->IsOffTheRecord() ? ui::DragDropTypes::DRAG_NONE
                                               : ui::DragDropTypes::DRAG_MOVE;
}

bool ExtensionsSuspendbarContainer::CanStartDragForView(View* sender,
                                                     const gfx::Point& press_pt,
                                                     const gfx::Point& p) {
...
bool ExtensionsSuspendbarContainer::GetDropFormats(
    int* formats,
    std::set<ui::ClipboardFormatType>* format_types) {
  return BrowserActionDragData::GetDropFormats(format_types);
}

bool ExtensionsSuspendbarContainer::AreDropTypesRequired() {
  return BrowserActionDragData::AreDropTypesRequired();
}

bool ExtensionsSuspendbarContainer::CanDrop(const OSExchangeData& data) {
  return BrowserActionDragData::CanDrop(data, browser_->profile());
}

void ExtensionsSuspendbarContainer::OnDragEntered(
    const ui::DropTargetEvent& event) {
  drop_weak_ptr_factory_.InvalidateWeakPtrs();
}

int ExtensionsSuspendbarContainer::OnDragUpdated(
    const ui::DropTargetEvent& event) {
...
void ExtensionsSuspendbarContainer::OnDragExited() {
  if (!drop_info_)
    return;
...
DragOperation ExtensionsSuspendbarContainer::OnPerformDrop(
    const ui::DropTargetEvent& event) {
...
views::View::DropCallback ExtensionsSuspendbarContainer::GetDropCallback(
    const ui::DropTargetEvent& event) {
  BrowserActionDragData data;
  if (!data.Read(event.data()))
  ...
  base::ScopedClosureRunner cleanup(
      base::BindOnce(&ExtensionsSuspendbarContainer::DragDropCleanup,
                     weak_ptr_factory_.GetWeakPtr(), action_id));
  return base::BindOnce(&ExtensionsSuspendbarContainer::MovePinnedAction,
                        drop_weak_ptr_factory_.GetWeakPtr(), action_id, index,
                        std::move(cleanup));
}

void ExtensionsSuspendbarContainer::OnWidgetClosing(views::Widget* widget) {
  auto iter = std::find_if(
      anchored_widgets_.begin(), anchored_widgets_.end(),
      [widget](const auto& info) { return info.widget == widget; });
...
void ExtensionsSuspendbarContainer::OnWidgetDestroying(views::Widget* widget) {
  OnWidgetClosing(widget);
}

size_t ExtensionsSuspendbarContainer::WidthToIconCount(int x_offset) {
...
gfx::ImageSkia ExtensionsSuspendbarContainer::GetExtensionIcon(
...
void ExtensionsSuspendbarContainer::SetExtensionIconVisibility(
  ...
  ToolbarActionView* extension_view = GetViewForId(*it);
  if (!extension_view || (extension_view->GetID() == 888888))
    return;

    auto icon_resize = gfx::ImageSkiaOperations::CreateResizedImage(
            GetExtensionIcon(extension_view),skia::ImageOperations::ResizeMethod::RESIZE_BEST,gfx::Size(80,80)
    );
...
void ExtensionsSuspendbarContainer::UpdateContainerVisibility() {
...
bool ExtensionsSuspendbarContainer::ShouldContainerBeVisible() const {
...
void ExtensionsSuspendbarContainer::UpdateContainerVisibilityAfterAnimation() {
  GetAnimatingLayoutManager()->PostOrQueueAction(
      base::BindOnce(&ExtensionsSuspendbarContainer::UpdateContainerVisibility,
                     weak_ptr_factory_.GetWeakPtr()));
}

void ExtensionsSuspendbarContainer::OnMenuOpening() {
  UpdateContainerVisibility();
}

void ExtensionsSuspendbarContainer::OnMenuClosed() {
  UpdateContainerVisibility();
}

void ExtensionsSuspendbarContainer::MovePinnedAction(
    const ToolbarActionsModel::ActionId& action_id,
    size_t index,
    base::ScopedClosureRunner cleanup,
    const ui::DropTargetEvent& event,
...
void ExtensionsSuspendbarContainer::DragDropCleanup(
    const ToolbarActionsModel::ActionId& dragged_extension_id) {
    //update on 20220427
  ReorderViews();
  GetAnimatingLayoutManager()->PostOrQueueAction(base::BindOnce(
      &ExtensionsSuspendbarContainer::SetExtensionIconVisibility,
      weak_ptr_factory_.GetWeakPtr(), dragged_extension_id, true));
}

void ExtensionsSuspendbarContainer::UpdateControlsVisibility() {
...
BEGIN_METADATA(ExtensionsSuspendbarContainer, ToolbarIconContainerView)
END_METADATA
