...
#ifndef CHROME_BROWSER_UI_VIEWS_EXTENSIONS_SCROLL_MENU_VIEW_H_
#define CHROME_BROWSER_UI_VIEWS_EXTENSIONS_SCROLL_MENU_VIEW_H_
...
class ScrollMenuView : public views::BubbleDialogDelegateView,
                           public TabStripModelObserver,
                           public ToolbarActionsModel::Observer {
 public:
  METADATA_HEADER(ScrollMenuView);
  ScrollMenuView(views::View* anchor_view,
                     Browser* browser,
                     ExtensionsContainer* extensions_container,
                     bool allow_pinning);
  ScrollMenuView(const ScrollMenuView&) = delete;
  ScrollMenuView& operator=(const ScrollMenuView&) = delete;
  ~ScrollMenuView() override;
  ...
  // Returns true if there is currently an ScrollMenuView showing (across
  // all browsers and profiles).
  static bool IsShowing();

  // Hides the currently-showing ScrollMenuView, if any exists.
  static void Hide();

  // Returns the currently-showing ScrollMenuView, if any exists.
  static ScrollMenuView* GetExtensionsMenuViewForTesting();
  ...
  // instances of the ScrollMenuView that are not created through
  // ShowBubble()).
 ...
 private:
  ...
  void SanityCheck();
   //update on 20220606
    void AddItemForWbview(Section section);
...
#endif  // CHROME_BROWSER_UI_VIEWS_EXTENSIONS_SCROLL_MENU_VIEW_H_
