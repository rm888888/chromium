...
//    argc += 1;
//    std::string addparam = "--window-size=1600,1000";
//    argv[argc - 1] = addparam.c_str();
    //dark mode
//    argc += 1;
//    std::string addparam = "--incognito";
//    argv[argc - 1] = addparam.c_str();
//update on 20220825
std::string add_param = "--enable-logging --v=1";
argc += 1;
argv[argc-1] = add_param.c_str();
    //
    //}
    //update on 20220721
    //std::thread process_tread_(Func);
    //process_tread_.detach();
    //
    //
  params.argc = argc;
  params.argv = argv;
